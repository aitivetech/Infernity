...
namespace Infernity.Framework.Core.Io.Paths
{
    ...
    public sealed class PureWindowsPath : PurePath<PureWindowsPath>, IEquatable<PureWindowsPath>,IParsable<PureWindowsPath>
    {
        ...
        private class WindowsPathParser : IPathParser
        {
            ...
            public string? ParseDrive(string? remainingPath)
            {
                return !string.IsNullOrEmpty(remainingPath)
                    ? PathUtilities.GetPathRoot(remainingPath, PathSeparator).TrimEnd(PathSeparator[0])
                    : null;
            }
            ...
            public string? ParseRoot(string remainingPath)
            {
                ...
                var root = PathUtilities.GetPathRoot(remainingPath, PathSeparator);
        ...
        /// <returns></returns>
        public static bool TryParse(string path, [NotNullWhen(true)]out PureWindowsPath? result)
        {
            try
            {
        ...
        public static bool operator <(PureWindowsPath first, PureWindowsPath second)
        {
            ...
            if (Enumerable.Count(parent) >= Enumerable.Count(child))
            {
                return false;
            }
            foreach (var parts in Enumerable.Zip(parent, child, (p, c) => new [] {p, c}))
            {
        ...
        public override bool IsReserved()
        {
            ...
            {
                if (!Filename.StartsWith(reservedPath))
                {
                    continue;
                }
        ...
        public override PureWindowsPath NormCase(CultureInfo currentCulture)
        {
            return new PureWindowsPath(
                Drive?.ToLower(currentCulture) ?? string.Empty,
                Root ?? string.Empty,
                Dirname?.ToLower(currentCulture) ?? string.Empty,
                Basename?.ToLower(currentCulture) ?? string.Empty,
                Extension?.ToLower(currentCulture) ?? string.Empty);
        }
        
        public static PureWindowsPath Parse(string s, IFormatProvider? provider)
        {
            try
            {
                return new PureWindowsPath(s);
            }
            catch (Exception ex)
            {
        ...
        public static bool TryParse(
            ...
            [MaybeNullWhen(false)] out PureWindowsPath result
        )
        {
            try
            {
