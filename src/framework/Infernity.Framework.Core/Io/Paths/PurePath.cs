...
namespace Infernity.Framework.Core.Io.Paths
{
        ...
        where TPath : PurePath<TPath>
    {
        ...
        protected PurePath()
        {
            ...
            Root = "";
            Dirname = "";
            Basename = PathUtilities.CurrentDirectoryIdentifier;
            Extension = "";
        }
        ...
        protected PurePath(IPathParser parser, params string[] paths)
        {
            ...
            if (paths.Length > 1)
            {
                var components = Enumerable.Select(paths, p =>
        ...
        private void Initialize(string? rawPath, IPathParser parser)
        {
            ...
            if (parser.ReservedCharactersInPath(rawPath, out var reservedCharacter))
            {
                throw new InvalidPathException(rawPath,
                    $"Path contains reserved character '{reservedCharacter}'.");
            }
        ...
        public string BasenameWithoutExtensions
        {
            get
            {
                var parts = Filename.Split(PathUtilities.ExtensionDelimiter);
                if (parts[0] == String.Empty && parts.Length > 1)
                {
                    // .dotfile is a filename, not an extension
                    return PathUtilities.ExtensionDelimiter + parts[1];
                }
                return parts[0];
            }
        }
        ...
        public string[] Extensions
        {
            get
            {
                var parts = Filename.Split(
                    [PathUtilities.ExtensionDelimiter],
                    StringSplitOptions.RemoveEmptyEntries);
                var ret = new string[parts.Length - 1];
                for (var i = 0; i < ret.Length; i++)
                {
                    ret[i] = PathUtilities.ExtensionDelimiter + parts[i + 1];
                }
                return ret;
            }
        }
        ...
        private readonly Lock _partsLock = new Lock();
        ...
        private readonly Lock _toPosixLock = new Lock();
        ...
        public TPath Join(params string[] paths)
        {
            ...
            return JoinInternal(
                new[] { (TPath)this }.Concat(paths.Select(PurePathFactory)));
        }

        public static TPath operator/ (PurePath<TPath> lvalue, PurePath<TPath> rvalue)
        {
            return lvalue.JoinInternal([lvalue, rvalue]);
        }

        public static TPath operator/ (PurePath<TPath> lvalue, string rvalue)
        {
            return lvalue.JoinInternal([lvalue, lvalue.PurePathFactory(rvalue)]);
        }
        ...
        {
            return JoinInternal(Enumerable.Concat(new[] { this }, paths));
        }
        ...
        {
            return JoinInternal(Enumerable.Select(paths, PurePathFactory));
        }

        private TPath JoinInternal(IEnumerable<TPath> paths)
        {
            return JoinInternal(Enumerable.Select(paths, p => (IPurePath)p));
        }
        ...
        private TPath JoinInternal(IEnumerable<IPurePath> paths)
        {
            ...
            var path = PurePathFactoryFromComponents(
                PathUtilities.Combine(pathsList, PathSeparator));
            if (path.Drive == String.Empty)
            {
                var filteredPaths = pathsList.Where(p => p.Drive != String.Empty)
                    .Select(p => p.Drive);

                // Need to retain the last drive since the Combine chops off
                // the drive if an absolute path comes along later.
                var drive = System.Linq.Enumerable.LastOrDefault(filteredPaths);
                if (drive != null)
                {
        ...
        /// <inheritdoc/>
        public bool TrySafeJoin(string relativePath, [NotNullWhen(true)]out TPath? joined)
        {
            var toJoin = PurePathFactory(relativePath);
            string? combined;
            if (!PathUtilities.TrySafeCombine(this, toJoin, PathSeparator, out combined))
            {
                joined = null;
                return false;
            }
        ...
        /// <inheritdoc/>
        public bool TrySafeJoin(IPurePath relativePath,[NotNullWhen(true)] out TPath? joined)
        {
            string? combined;
            if (!PathUtilities.TrySafeCombine(this, relativePath, PathSeparator, out combined))
            {
                joined = null;
                return false;
            }
        ...
        private string NormalizeSeparators(string path)
        {
            ...
            foreach (var separator in PathUtilities.PathSeparatorsForNormalization)
            {
        ...
        private void Normalize()
        {
            ...
            newDirname = Regex.Replace(newDirname, String.Format(
                ...
                Regex.Escape(PathUtilities.CurrentDirectoryIdentifier)),
                                       @"$2");
        ...
        {
            var maxPathLength = Enumerable.Count(Parts) - 1;  // Don't return self as a parent
            for (var i = maxPathLength; i > 0; i--)
            {
                yield return PurePathFactory(
                    JoinInternal(
                        Enumerable.Take(Parts, i))
                    .ToString());
            }
        }
        ...
        {
            return Enumerable.Select(Parents(), p => (IPurePath)p);
        }
        ...
        public TPath RelativeTo(IPurePath parent)
        {
                ...
                !ComponentComparer.Equals(parent.Root, Root))
            {
                throw new ArgumentException(
                    $"'{this}' does not share the same root/drive as '{parent}', " +
                    "thus cannot be relative.");
            }

            var thisDirname = (Dirname ?? string.Empty)
            ...
            while (parentDirname.MoveNext())
            {
                ...
                {
                    throw new ArgumentException($"'{this}' does not start with '{parent}'");
                }
            }
        ...
        public TPath WithExtension(string newExtension)
        {
            ...
            if (fname.HasComponents(PathComponent.Extension))
            {
                ...
                return PurePathFactoryFromComponents(this,
                    basename: Basename + PrependWithDot(fname.Basename ?? string.Empty),
                    extension: PrependWithDot(fname.Extension ??string.Empty));
            }

            return PurePathFactoryFromComponents(this,
                extension: PrependWithDot(fname.Basename ?? string.Empty));
        }

        private static string PrependWithDot(string extension)
        {
            if (extension.StartsWith("" + PathUtilities.ExtensionDelimiter))
            {
                return extension;
            }
            return PathUtilities.ExtensionDelimiter + extension;
        }
        ...
        public TPath WithFilename(string newFilename)
        {
            ...
            {
                throw new ArgumentException(
                    $"New filename '{newFilename}' must contain only basename and/or extension.",
                    nameof(newFilename));
            }
            return PurePathFactoryFromComponents(this,
        ...
        public string GetComponents(PathComponent components)
        {
                ...
                && Basename != String.Empty)
            {
                path = !String.IsNullOrEmpty(path)
                    ? PathUtilities.Combine(path, Basename?? string.Empty, PathSeparator)
                    : Basename;
            }
            if ((components & PathComponent.Extension) == PathComponent.Extension)
            {
        ...
        */
        ...
        {
            return PurePathFactoryFromComponents(
                (drive ?? (original != null ? original.Drive : "")) ?? string.Empty,
                (root ?? (original != null ? original.Root : "")) ?? string.Empty,
                (dirname ?? (original != null ? original.Dirname : "")) ?? string.Empty,
                (basename ?? (original != null ? original.Basename : "")) ?? string.Empty,
                (extension ?? (original != null ? original.Extension : "")) ?? string.Empty);
        }
