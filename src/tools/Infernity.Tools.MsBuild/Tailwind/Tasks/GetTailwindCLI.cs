...
namespace Infernity.Tools.MsBuild.Tailwind.Tasks;
...
public class GetTailwindCLI : Microsoft.Build.Utilities.Task
{
    ...
    public override bool Execute()
    {
        try
        {
            ...
            if (File.Exists(this.StandaloneCliPath))
            {
                this.Log.LogMessage(MessageImportance.Low,
                    "Using cached tailwind binary '{0}'",
                    this.StandaloneCliPath);
        ...
        {
            var targetsFile = Path.Combine(Path.GetDirectoryName(this.BuildEngine.ProjectFileOfTaskNode),
                "build",
                "Tailwind.MSBuild.targets");
    ...
    public string GetFilePath()
    {
        ...
        var arch = Microsoft.Build.Utilities.ProcessorArchitecture.CurrentProcessArchitecture;

        if (arch == Microsoft.Build.Utilities.ProcessorArchitecture.AMD64)
        {
            arch = "x64";
        }
        else
        ...
        return Path.GetFullPath(Path.Combine(this.RootInstallPath,
            this.Version,
            fileName));
    }
    ...
    private void DownloadCli(string fileName)
    {
        ...
        var asset = (release?.Assets.FirstOrDefault(a => a.Name.Equals(fileName,
                    ...
                    ?? throw new Exception(
                        $"Unable to find a download link for '{fileName}' with Tailwind version '{this.Version}'");
        ...
        Directory.CreateDirectory(
            Path.GetDirectoryName(this.StandaloneCliPath) ?? throw new InvalidOperationException());
